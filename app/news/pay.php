...
final class webapp_pay_zz implements webapp_pay
{
	static function paytype():array
	{
		return [
	...
	function create(array &$order, ?string &$error):bool
	{
		do
		{
			...
			//print_r($data);
			if (is_array($result = webapp_client_http::open('https://zzzfwg900.xxpp1122.club/api/pay/create_order', [
				'timeout' => 8,
				'autoretry' => 2,
				'method' => 'POST',
			...
			$order['trade_no'] = preg_match('/order_id=(\d+)/', $result['payUrl'], $id) ? $id[1] : $order['hash'];
			$order['type'] = 'goto';
...
final class webapp_pay_kk implements webapp_pay
{
	static function paytype():array
	{
		return [
	...
	function create(array &$order, ?string &$error):bool
	{
		do
		{
			...
			$data['sign'] = strtoupper(md5(join('&', $query)));
			//print_r($data);
			if (is_array($result = webapp_client_http::open('http://pay.ishangf.com/api/pay/create_order', [
				'timeout' => 8,
				'autoretry' => 2,
				'method' => 'POST',
				//'type' => 'application/json',
			...
			//var_dump($result);
			if ((isset($result['retCode'], $result['payOrderId'], $result['payJumpUrl']) && $result['retCode'] === '0') === FALSE)
			{
				$error = '远程支付失败！';
				break;
			}
			$order['trade_no'] = $result['payOrderId'];
			$order['type'] = 'goto';
			$order['data'] = $result['payJumpUrl'];
			return TRUE;
		} while (0);
		return FALSE;
	}
...
final class webapp_pay_kj implements webapp_pay
{
	static function paytype():array
	{
		return [
	...
	function create(array &$order, ?string &$error):bool
	{
		do
		{
			...
			$data['sign'] = strtoupper(md5(join('&', $query)));
			//print_r($data);
			if (is_array($result = webapp_client_http::open('https://keji13keji13.333999.site/api/pay/create_order', [
				'timeout' => 8,
				'autoretry' => 2,
				'method' => 'POST',
				//'type' => 'application/json',
